...
import { MusicNotes, Play, CheckCircle, ArrowCounterClockwise } from '@phosphor-icons/react';
...
const generateRandomSequence = () => {
  const sequence = NOTE_STEPS.map((note, index) => ({
    ...note,
    stepIndex: index,
  }));
...
export function MusicCleaning() {
  const componentId = useId();
  const [sequence, setSequence] = useState(generateRandomSequence());
  const [currentRound, setCurrentRound] = useState(0);
  const [placedNote, setPlacedNote] = useState<PlacedNote | null>(null);
  ...
  const playTargetNote = useCallback(async () => {
    ...
    const currentNote = sequence[currentRound];
    const fullNote = `${currentNote.name}${currentNote.octave}`;
  ...
  }, [sequence, currentRound, stopAudio, componentId]);

  const handleStaffClick = async (e: React.MouseEvent) => {
    if (isGameWon || placedNote?.status === 'correct' || !staffRef.current) return;
    ...
    const targetNote = sequence[currentRound];
    const isCorrect = closestStep === targetNote.stepIndex;
    const clickedNoteBase = NOTE_STEPS[closestStep];
    const fullNote = `${clickedNoteBase.name}${clickedNoteBase.octave}`;
  ...
  useEffect(() => {
    if (currentRound > 0 && currentRound < TOTAL_ROUNDS && !isGameWon) {
      const timer = setTimeout(() => playTargetNote(), 500);
      return () => clearTimeout(timer);
    }
  }, [currentRound, isGameWon, playTargetNote]);
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-4">
        <div className="flex flex-col items-center px-2 text-center">
          ...
          <p className="mt-1 text-sm text-text/40">
            Послушай ноту и найди её на стане. Раунд {currentRound + 1} из {TOTAL_ROUNDS}.
          </p>
        </div>

        <div className="relative w-full scrollbar-none overflow-x-auto rounded-xl border border-text/10 bg-background/50 select-none">
          <div
            ref={staffRef}
          ...
          >
            ...
            {placedNote && (
              <div
                ...
                style={{ left: `${placedNote.x}px`, top: `${getYForStep(placedNote.stepIndex)}px` }}
              >
                {placedNote.stepIndex === 0 && (
                  <div className="absolute -top-[1px] -left-[16px] h-[2px] w-[32px] bg-text/60" />
                )}
                <div
                  className={cn(
                    'absolute -top-[6px] -left-[9px] h-[12px] w-[14px] -rotate-15 rounded-[50%] shadow-sm transition-colors duration-300',
                    placedNote.status === 'correct' ? 'bg-access' : 'bg-red-500',
                  )}
                />
                <div
                  className={cn(
                    'absolute w-[2px] transition-colors duration-300',
                    placedNote.status === 'correct' ? 'bg-access' : 'bg-red-500',
                  )}
                  style={{
                    height: '35px',
                    left: placedNote.stepIndex >= 6 ? '-9px' : '3px',
                    top: placedNote.stepIndex >= 6 ? '-1px' : '-35px',
                  }}
                />
              </div>
            )}
        ...
        <div className="flex w-full items-center justify-center px-2">
          <Button
            ...
            color="primary"
            className="gap-2"
            disabled={isPlayingSound || isGameWon}
          >
            <Play size={20} />
            {!hasPlayedOnce ? 'Слушать ноту' : 'Повторить ноту'}
          </Button>
        </div>
      </div>

      {isGameWon && (
        <div className="animate-in fade-in absolute inset-0 z-50 flex flex-col items-center justify-center bg-background/85 backdrop-blur-lg duration-500">
          <div className="flex flex-col items-center gap-4 px-4 text-center text-access">
            ...
            <div>
              ...
              <p className="text-sm text-text/70">Ты угадал все 7 нот.</p>
            </div>
            <Button
              variant="outline"
              color="primary"
