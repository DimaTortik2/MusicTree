...
export const DetailLayout: React.FC<DetailLayoutProps> = ({
  ...
  return (
    <div className="relative flex h-full w-full overflow-hidden bg-background font-sans">
      ...
      <div
        className={cn(
          'flex h-full w-full flex-col border-surface md:w-[360px] md:border-r lg:w-[420px]',
          isMobileDetailOpen ? 'hidden md:flex' : 'flex',
        )}
      >
        <div ref={listRef} className="flex-1 overflow-y-auto px-4 py-8 pb-32 md:pb-8">
          {listContent}
        </div>
      </div>
      ...
      >
        ...
        {/* 🔥 ВЕШАЕМ REF НА КОНТЕЙНЕР СО СКРОЛЛОМ */}
        <div
          ref={detailRef}
          className="flex flex-1 flex-col overflow-y-auto px-6 pt-4 pb-12 md:px-16 md:pt-16"
        >
          {detailContent}
        </div>
      </div>
