...
export function useVocalTuner() {
  ...
  const sharedTreeId = useAppModeStore((s) => s.sharedTreeId);
  ...
  useEffect(() => {
    // ШВЕЙЦАРСКИЕ ЧАСЫ: Ждем, пока стор авторизации вообще запустится
    if (!authInitialized) return;

    // Если юзер есть, строго ЖДЕМ, пока не прилетит его профиль из БД!
    if (user && !profile) return;

    // Если записи уже загружены - прерываем
    if (hasLoaded) return;

    const loadRecordings = async () => {
      const isSharedMode = !!sharedTreeId;
      const currentTreeId = sharedTreeId;

      // Теперь profile 100% не null, если мы залогинены
      const userHasCloudAccess = !!profile?.can_cloud_audio;
      ...
      if (userHasCloudAccess && user) {
        ...
        if (isSharedMode) {
          query = query.eq('shared_tree_id', currentTreeId);
        } else {
          query = query.eq('user_id', user.id).is('shared_tree_id', null);
        }
        ...
        // ЛОКАЛЬНЫЙ РЕЖИМ (с подтягиванием чужого облака, если мы в дереве)
        ...
        if (isSharedMode && currentTreeId) {
          const { data: friendTracks } = await supabase
            .from('audio_tracks')
            .select('*')
            .eq('shared_tree_id', currentTreeId)
            .order('created_at', { ascending: false });

          if (friendTracks && friendTracks.length > 0) {
            const paths = friendTracks.map((track) => track.url);
            const { data: signedUrlsData } = await supabase.storage
              .from('audio_records')
              .createSignedUrls(paths, 86400);

            const cloudLoaded: Recording[] = friendTracks.map((track, index) => ({
              id: track.id,
              name: track.title,
              time: new Date(track.created_at).toLocaleDateString(),
              url: signedUrlsData?.[index]?.signedUrl || '',
    ...
    // Добавили реактивные зависимости, чтобы хук "проснулся", когда прилетит профиль
  }, [
    authInitialized,
    user,
    profile,
  ...
  const startMic = async () => {
    ...
    try {
      ...
      if (sr < 44100 && !sampleRateWarnedRef.current) {
        toast.error('Вы используете Bluetooth-устройство или iOS. Точность может быть снижена.', {
          style: { opacity: 0.8 },
          position: 'bottom-right',
        });
      ...
      if (err.message === 'InsecureContext') setMicError('denied');
  ...
  const startRec = () => {
    ...
    mr.onstop = async () => {
      ...
      if (isCloudMode && user) {
        const isSharedMode = !!useAppModeStore.getState().sharedTreeId;
        const currentTreeId = useAppModeStore.getState().sharedTreeId;
        ...
        try {
          ...
          const { error: dbError } = await supabase.from('audio_tracks').insert([
            {
              id: uuid,
              user_id: user.id,
              shared_tree_id: isSharedMode ? currentTreeId : null,
              title,
              dur,
              url: filePath,
          ...
          toast.dismiss(toastId);
          toast.error('Ошибка сохранения: ' + e.message, { position: 'bottom-right' });
        } finally {
        ...
        try {
          ...
          if (e.name === 'QuotaExceededError')
            toast.error('Недостаточно памяти.', { position: 'bottom-right' });
        } finally {
          useVocalGlobalStore.getState().setIsSaving(false);
        }
      }
  ...
  const togglePlay = async (rec: Recording) => {
    ...
    if (!isCloudMode) {
      ...
      if (!exists) {
        toast.error('Аудиофайл очищен системой', { position: 'bottom-right' });
      ...
      if (isCloudMode) {
        ...
        if (user) {
          ...
          await supabase.storage
            .from('audio_records')
            .remove([`${user.id}/${urlParts[urlParts.length - 1]}`]);
        }
      } else {
  ...
  const deleteRec = (rec: Recording) => {
    ...
    const toastId = toast.undo(
      <div className="flex w-full items-center justify-between gap-3">
        ...
        <Button
          ...
          onClick={(e) => {
            ...
            toast.dismiss(toastId);
            setRecordings((prev) => [...prev, rec].sort((a, b) => b.createdAt - a.createdAt));
          }}
        >
          Отменить
        </Button>
      </div>,
      {
        ...
        position: 'bottom-right',
        onClose: () => {
          if (undoneDeletionsRef.current.has(rec.id)) undoneDeletionsRef.current.delete(rec.id);
          else hardDeleteRec(rec);
        },
      },
    );
  };
  ...
  const downloadRec = async (rec: Recording) => {
    ...
    try {
      ...
      toast.error('Ошибка при скачивании', { position: 'bottom-right' });
    }
  };
  ...
  const renameRec = async (id: string, newName: string) => {
    ...
    try {
      ...
      toast.error('Ошибка при переименовании', { position: 'bottom-right' });
    }
  };
